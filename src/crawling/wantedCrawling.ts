...
export async function login(
  ...
  ID,
  PW,
): Promise<[page: Page, browser: Browser, isSuccess: boolean]> {
  
  const userAgent =
    'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_5) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.61 Safari/537.36';
  const context = await browserCenter.newContext({ userAgent });
  ...
  try {
    ...
    return [page, browserCenter, true];
  } catch (error) {
    console.log(error);
    return [page, browserCenter, false];
  }
}
...
export async function wantedCrawling(browserCenter,tmpdir, ID, PW) {
  // Log in
  const [page, browser, isSuccess] = await login(browserCenter,ID, PW);

  if (!isSuccess) {
    await navigateJobPostings(page);
    await getJobPostings(page);
    let allUserInfo = [];

    for (let postId of ['174982']) {
      const userInfoByJobPosting = await saveUserResume(tmpdir, page, postId);
